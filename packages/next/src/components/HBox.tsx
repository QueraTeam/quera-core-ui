...
export const HBox = ({
  ...
  <HStack spacing={[4, null, 6]} align="stretch">
    <Box>
      <ConditionalLink display="block" href={url} isNextLink={isNextLink} isLink={url !== "" && url != null}>
        {image}
      </ConditionalLink>
      {imageExtra}
    </Box>
    <Stack flex={1} minW={0} spacing={size === "md" ? [1.5, null, 2] : 0.5}>
      <Flex align="center">
        ...
        <Heading size={size} lineHeight="tall" marginEnd={tag && 3}>
          <ConditionalLink href={url} isNextLink={isNextLink} isLink={url !== "" && url != null}>
            {title}
          </ConditionalLink>
        </Heading>
        <Spacer />
