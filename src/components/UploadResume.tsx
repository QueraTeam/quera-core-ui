...
export interface UploadResumeProps<TFieldValues extends FieldValues> extends BoxProps {
...
export const UploadResume = <T,>({
  ...
  const previousFileExists = previousFile && previousFile.name && previousFile.size;
  const [uploadMode, setUploadMode] = React.useState<boolean>(!previousFileExists);
  ...
  return (
    <Controller
      ...
      render={({ field: resumeTypeField }) => (
        <RadioGroup {...resumeTypeField} {...rest} defaultValue={resumeTypes.UPLOAD}>
          <VStack spacing={6}>
            <VStack align="flex-start" width="full">
              ...
              <Controller
                ...
                control={control}
                rules={{
                  required: resumeTypeField.value === resumeTypes.UPLOAD && uploadMode,
                }}
                render={({ field: fileInputField, fieldState }) => (
                  <FormControl
                    isInvalid={!!fieldState.error}
                    opacity={resumeTypeField.value === resumeTypes.UPLOAD ? 1 : 0.3}
                    pointerEvents={resumeTypeField.value === resumeTypes.UPLOAD ? "auto" : "none"}
                  >
                    <FileInputField
            ...
            <VStack align="flex-start" width="full">
              ...
              <Box
                w="full"
                pointerEvents={resumeTypeField.value === resumeTypes.QCV ? "auto" : "none"}
