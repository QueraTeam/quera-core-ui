...
const ReachedPageBottom = createContext(false);

interface CardHeaderProps extends HeadingProps {
  subtitle?: string;
}

export const CardHeader = forwardRef<CardHeaderProps, "h2">(
  ({ as = "h2", subtitle, ...rest }, ref) => {
    const styles = useStyles();
    const heading = (
      <chakra.header as={as} ref={ref} __css={styles.header} {...rest} />
    );
...
interface CollapsibleCardProps extends CardProps {
  collapsible?: boolean;
  collapsed?: boolean;
  onCollapseToggle?: MouseEventHandler;
}
export const CollapsibleCard = ({
  ...
  <Card {...rest}>
    {children}
    {collapsible && (
      <CardButton onClick={onCollapseToggle}>
        {collapsed ? "مشاهده بیشتر" : "مشاهده کمتر"}
      </CardButton>
    )}
  </Card>
);
...
export const FixedBottomCard = ({ children, ...rest }: CardProps) => {
  const reachedPageBottom = useContext(ReachedPageBottom);
  return (
    <Card
      position="fixed"
      borderRadius={0}
