...
import { Button, ButtonProps, HStack, Spinner, Text, Tooltip } from "@chakra-ui/react";
...
export const PinButton = ({ pinned, controlled = false, hintType = "tooltip", ...buttonProps }: PinButtonProps) => {
  const [isPinning] = React.useState(false);
  const [_isPinned, setIsPinned] = React.useState(pinned);
  ...
  const icon = isPinned ? <MdIcon icon={mdiBookmark} boxSize={6} /> : <MdIcon icon={mdiBookmarkOutline} boxSize={6} />;
